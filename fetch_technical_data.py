...
def fetch_technical_data_to_csv(symbols_file, start_date="2020-01-01", end_date=None, output_file="stock_data_technical.csv"):
    ...
    Processes symbols in batches of 10 with a 1-second delay between batches to avoid rate limiting.
    """
    if end_date is None:
    ...
    # Buffer for technical indicators
    buffer_days = 60
    ...
    # Process symbols in batches of 10 with 1-second delay
    batch_size = 10
    for i in range(0, len(symbols), batch_size):
        batch_symbols = symbols[i:i + batch_size]
        print(f"Processing batch {i//batch_size + 1} of {len(symbols)//batch_size + 1}: {batch_symbols}")
        ...
        for symbol in batch_symbols:
            ...
            try:
                ...
                print(f"Failed to fetch info for {symbol}: {e}, skipping fundamentals")
                df_history['MarketCap'] = np.nan
            ...
            df_history['Price_Change'] = df_history['Close'].diff().fillna(0)
        ...
        if i + batch_size < len(symbols):  # Only sleep if there's another batch
            print(f"Pausing for 1 second to avoid rate limiting...")
            time.sleep(1)
